...
class Game_options(Game_Setup):
    def play_with_computer(self, num_rounds, difficulty):
        while True:
            ...
            for round in range(1, num_rounds + 1):
                ...
                results.append(f"\nRound {round}: You chose {player_choice}, Computer chose {computer_choice}. Result: {result}")
            return "\n".join(results)
    ...
    def player_choices(self):
        while True:
            player_choice = getpass.getpass("> Enter your choice (rock, paper, scissors, lizard, spock) >: ").lower()
